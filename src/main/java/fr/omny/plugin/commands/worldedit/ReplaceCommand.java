...
public class ReplaceCommand extends Cmd {
	...
	public ReplaceCommand() {
		...
		rc(1, new BlockArgument(false));
	}

	@Override
	public void execute(CommandSender sender, Arguments args) {
		if (sender instanceof Player player) {
			var oldType = args.get(0, Material.class);
			var newType = args.get(1, Material.class);
			...
			var blockUpdates = area.getBlockUpdate()
					.stream()
					.filter(BlockUpdate.filter(oldType))
					.map(BlockUpdate.setType(newType))
